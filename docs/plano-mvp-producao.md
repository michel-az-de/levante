...
**noindex no host provisório.** No host provisório da VM (`sslip.io`, `Host` ≠ `SITE_HOST`), suprimir a indexação **de verdade**: `X-Robots-Tag: noindex` **+ `robots.txt` `Disallow: /` + `sitemap` vazio/404**, todos **condicionais ao header `Host`**, via middleware. `X-Robots-Tag` de página HTML não impede o bot de puxar `sitemap.xml`/OG — por isso robots e sitemap também. A mesma app serve o domínio final pós-cutover, e um estado fixo vazaria para ele.
