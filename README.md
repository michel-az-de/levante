...
**Fase C — em andamento** (C1, outbox): eventos de domínio (`ArtigoPublicado`, `ComentarioCriado`, `ComentarioAprovado`) agora vão a um **Outbox transacional** e um relay (reconciliação por polling → RabbitMQ) os publica para o **Hiram** entregar. Envelope provisório documentado em [`docs/adr/0001-outbox-envelope-hiram.md`](docs/adr/0001-outbox-envelope-hiram.md). A escrita usa transação onde há replica set (Atlas); em single-node degrada para sequencial (dev/test).
