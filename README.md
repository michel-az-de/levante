...
**C2 — Audiência (newsletter double opt-in)**: contexto novo `Levante.Audiencia` (agregado `Assinante`). Inscrição pública anônima (`POST /newsletter`, honeypot + rate limit) nasce **Pendente** e gera `AssinaturaSolicitada` no Outbox → o **Hiram** envia o e-mail de confirmação (o site nunca chama provedor de e-mail — regra 7). O link confirma por **token opaco** (`/newsletter/confirmar`) → **Confirmado**; o descadastro (`/newsletter/cancelar`) é o opt-out. **LGPD**: consentimento com timestamp, e-mail guardado com finalidade explícita, e a resposta pública é sempre a mesma (não revela se um e-mail já existe). Front: `/newsletter` (form), `/newsletter/confirmar` e `/newsletter/cancelar` (via BFF público). Isso já entrega a **notificação de comentário pendente** da C3 (o `ComentarioCriado` flui pelo mesmo Outbox).
